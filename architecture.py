...
class ContextMatcher(nn.Module):
    ...
    def __init__(self, embeddings, elmo, LM, candidate_map=None):
        ...
        if isinstance(self.LM, nn.Module):
            freeze(self.LM)
...
class PointerGenerator(nn.Module):
    ...
    def forward(self, src, src_mask, max_len, start_symbol, mode = 'sample'):
        ...
        for i in range(max_len):
            ...
            log_probs = final_dis.log() #(batch, vocab)
        ...
        all_log_probs = torch.stack(all_log_probs, dim=1) #(batch, seq, vocab)
        # if self.coverage:
        #     return ys[:,1:], log_probs_seq, covloss
        # return ys[:,1:], log_probs_seq
        if self.coverage:
            return ys[:,1:], all_log_probs, covloss
        return ys[:,1:], all_log_probs
